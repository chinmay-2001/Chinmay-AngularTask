...
import { map, filter, interval, range, concat, take } from 'rxjs';
...
export class RxjsTaskComponent {
  ...
  getProd() {
    let product = of({ pName: "televison", price: 17999 },
    ...
    product.pipe(map((state) => state.pName)).subscribe(ev => this.productNameList.push(ev))
    //2.Display sum of price  all products whose price is  >30000
    product.pipe(filter((state) => state.price > 30000)).subscribe(ev => this.productlist.push(ev))
    ...
    // candidates.pipe(map(state=>state)).subscribe(ev=>{if(ev.)})
